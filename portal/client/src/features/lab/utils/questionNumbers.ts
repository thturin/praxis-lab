 ...
 *
 * Used in LabPreview and QuestionBlock components to show human-friendly
 * question numbers to students.
 *
 * @param blocks - Array of blocks from lab.blocks
    ...
    if (block.blockType === 'question') {
      ...
      if (block.subQuestions?.length > 0) {
        block.subQuestions.forEach((subQ, subIdx) => {
          ...
          displayMap[subQ.id] = `${questionIndex}${letter}`;
        });
      }
    }
  });
